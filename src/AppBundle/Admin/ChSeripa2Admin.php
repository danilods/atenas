...
class ChSeripa2Admin extends AbstractAdmin{
    //put your code here
   // protected $emName = 'unidade';
     protected $baseRouteName = 'os_seripa2';
     protected $baseRoutePattern = 'page-ch-seripa2';
    ...
    protected function configureRoutes(RouteCollection $collection)
    {
		...
		$collection->add('history', $this->getRouterIdParameter().'/history');
	 	 ...
	 	 //carregar somente os dados de cada unidade
	 ...
	 public function createQuery($context = 'list') {
			...
			$user = $this->getConfigurationPool()->getContainer()->get('security.context')->getToken()->getUser();
				...
				$query->innerJoin($query->getRootAlias().'.omPagadora','om')
					  ...
					  ->setParameter('unidade', 3)
					  ->setParameter('status', 'ANALISADA')
	...
	protected function configureFormFields(FormMapper $formMapper)
    {
     $formMapper
			...
			->add('diariasCompletas',null, array(
					
					'label' => 'Diárias completas:',
					'required'=>false 
					))
           ...
           ->with('AUTORIZAÇÃO DE DIÁRIAS', array(
            ...
            ->add('statusOs', 'choice', array(
                'choices' => array(
                    ...
                    'ANALISADA' => 'ORDEM DE SERVIÇO EMITIDA PELO SETOR ADMINISTRATIVO',
					'AUTORIZADA' => 'AUTORIZAR ABERTURA DE ORDEM DE SERVIÇO',
					'NÃO AUTORIZADA' => 'NÃO AUTORIZAR ABERTURA DE ORDEM DE SERVIÇO',

					'CANCELADA' => 'CANCELAR/NÃO AUTORIZAR ORDEM DE SERVIÇO',
           ...
           ->with('EMISSÃO DE PASSAGENS', array(
            ...
            ->add('statusPassagem', 'choice', array(
                'choices' => array(
                    ...
                    '1' => 'EMISSÃO DE PASSAGEM SOLICITADA PELO SETOR ADMINISTRATIVO',
                    '2' => 'APROVAR EMISSÃO DE PASSAGEM E ENVIAR AO CENIPA',
    ...
    protected function configureListFields(ListMapper $listMapper)
    {
        ...
        ->add('numeroOrdemServico', null, array('template' => 'SonataAdminBundle:CRUD:custom_list_numero_os.html.twig', 'label' => 'Número da OS'))
			...
			->add('statusPassagem', null, array('template' => 'SonataAdminBundle:CRUD:custom_list_status_passagem_seripa.html.twig', 'label' => 'Status da passagem'))
            ...
            ->add('_action', 'actions', array(
                'actions' => array(
					...
					'Histórico de alterações' => array('template' => 'button_history.html.twig'),
 ...
 public function preUpdate($object) {
		...
		if($object->getStatusOs()=='AUTORIZADA' && $object->getStatusPassagem()==1){
			$object->setStatusPassagem(2);
		}		
				  ...
				  foreach ($object->getAeroporto() as $aeroporto) {
					...
					foreach ($object->getRoteiro() as $roteiro) {
						...
						$arrayDatasRetorno[] =  $roteiro->getDateTermino()->format('Y-m-d');
		 ...
		 if($roteirosMesmaData[0] == count($object->getRoteiro())){
					...
					//$dataIda,  $dataRetorno, $usuario, $cidade, $deslocamento, $meia, $datasIguais
					 ...
					 $roteiroPernoite[0]->setQuantidadeDiarias(0.0);
					 $roteiroPernoite[0]->setValorDiaria(0.0);
					 ...
					 if($count > 0){
						  ...
						  $saidaDemais = $this->calcularDiariasTrecho($dataIda,$dataRetorno, $usuario, $cidade,$roteirosRestantes->getAdicionalDeslocamento(), $meia=0.0, $datasIguais=0);
						 
						  $roteirosRestantes->setQuantidadeDiarias($saidaDemais[1]);
						  $roteirosRestantes->setValorDiaria($saidaDemais[0]);
		 ...
		 $custo__estimado = 0;
				foreach ($object->getRoteiro() as $roteiro) {
					...
					$desconto = $horaInicioPeriodo < $horaLimiteDescontoAuxilio ? 0 : 1; //se o horário for anterior às 18 horas, vai descontar nas diárias da OS
					
					
					
					$idUser = $object->getTbUsuario()->getId();
					
					$idaPeriodo = $arrayDatas[0];
					
					$voltaPeriodo = $arrayDatasRetorno[count($arrayDatasRetorno)-1];
					
					$controller = new GestorController();
					
					$quantidadeDiasUteisRoteiro = $controller->getWorkingDays($idaPeriodo, $voltaPeriodo);
						
						
					$calculoAuxilio = $this->calcularAuxilio($idUser, $quantidadeDiasUteisRoteiro);						
				...
				if($object->getMissaoCusto() == 1){
				
					$object->setAuxilioAlimentacao(0.00);
			
					$object->setAuxilioTransporte(0.00);
					...
					$diariasCompletas = $quantidade__diarias == 0.5 ? 0.5 : $quantidade__diarias-0.5;
					
					$object->setDiariasCompletas($diariasCompletas);
					...
					$object->setQuantidadeDiarias($quantidade__diarias);
						...
						$custo_total = $custo__estimado - $totalDescontos;
		...
		public function calcularAuxilio($idUsuario, $diasUteis){
				...
				$saida1 = round($diarioAlimentacao, 2);
				
				$saida2 = round($diarioTransporte, 2);
				
				$diasUteisAlimentacao = $diasUteis;
				
				$valorAlimentacaoRoteiro = $saida1 * $diasUteisAlimentacao;
				
				$valorTransporteRoteiro = $saida2 * $diasUteis;
		...
		public function getRoteirosComPernoite($idOs){
				...
				if(count($roteirosComPernoite)>0){
				//demais roteiros
					$queryRot2 = $repositoryRoteiro->createQueryBuilder('rot')
					 ->where('rot.ordemServico = :osId and rot.id != :id')
					 ->setParameter('osId', $idOs)
					...
					return array($roteirosComPernoite[0],0);
				}
