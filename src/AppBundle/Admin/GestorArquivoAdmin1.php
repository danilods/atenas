...
class GestorArquivoAdmin1 extends AbstractAdmin{
    //put your code here
    // protected $emName = 'unidade';
    use AdminExtraExportTrait;

    protected $baseRouteName = 'arquivo_seripa1';
    protected $baseRoutePattern = 'page-arquivo-seripa1';
    ...
    $user = $this->getConfigurationPool()->getContainer()->get('security.context')->getToken()->getUser();
        ...
        $query->innerJoin($query->getRootAlias().'.omPagadora','om')
              ...
              ->where('om.id = :unidade')
			  ->andWhere($query->getRootAliases()[0]. '.statusOs = :status or o.statusOs = :status2 or o.statusOs = :status3')
			  
			  ->setParameter('unidade', 2)
			  ->setParameter('status', 'CANCELADA')
			  ->setParameter('status2', 'PROCESSADA')

				->setParameter('status3', 'NAO AUTORIZADA')
    ...
    protected function configureFormFields(FormMapper $formMapper)
    {
        $formMapper
            ...
            ->add('statusOs', 'choice', array(
                'choices' => array(
                    'PENDENTE' => 'ORDEM DE SERVIÇO PENDENTE DE APROVA��O',
