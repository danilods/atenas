...
class GestorAdminExterior extends AbstractAdmin{
    //put your code here
    ...
    $user = $this->getConfigurationPool()->getContainer()->get('security.context')->getToken()->getUser();
        ...
        $query->innerJoin($query->getRootAlias().'.omPagadora','om')
              ...
              ->where('om.id = :unidade')
			  ->andWhere($query->getRootAliases()[0]. '.statusOs != :status and o.statusOs != :status2 and o.tipoMissao != :tipoMissao')		  			  
			  ->setParameter('unidade', 1)
			  ->setParameter('status', 'CANCELADA')
			  ->setParameter('status2', 'PROCESSADA')
			  ->setParameter('tipoMissao', 'NACIONAL')
  ...
  public function prePersist($object) {
   ...
   public function preUpdate($object) {
        ...
        foreach ($object->getRoteiroExterior() as $roteiro) {
			...
			$object->setDiariasCompletas($quantidadeDiarias-0.5);
